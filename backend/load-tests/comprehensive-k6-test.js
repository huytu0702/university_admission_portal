...
export const options = {
  ...
  thresholds: {
    ...
    // Checks should pass at least 99% of the time
    'checks': ['rate>0.99'],
  },
};
...
// Soak test scenario - Longer duration with consistent load
export function soakScenario() {
  ...
  // Repeatedly submit applications and process payments
  for (let i = 0; i < 5; i++) {
    if (authToken) {
      const applicationRes = submitApplication(authToken);
...
function submitApplication(authToken) {
  ...
  check(res, {
    'application submission status is 201': (r) => r.status === 201,
